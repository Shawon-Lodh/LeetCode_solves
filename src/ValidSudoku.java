...
//        System.out.println("Column checking validity is : "+ checkBasedOnRowOrColumn(getTransposeMatrix(board)));
        ...
        while(rowGap != board.length){
            ...
            for(int i = 0; i<3; i++){
                for(int j = 0; j<3; j++){
                    copy[i][j] = board[rowGap+i][colGap+j];
                }
            }
...
//        }
        ...
        for(int i = 0;i<boardCubePart.length;i++){
            for(int j = 0;j<boardCubePart.length;j++){
               if(set.contains(boardCubePart[i][j]) && boardCubePart[i][j] != '.'){
                  return false;
               }
               set.add(boardCubePart[i][j]);
            }
        }
        return true;
    }
...
//            System.out.println();
    ...
    public static void main(String[] args) {
       ...
       ValidSudoku solutionObj = new ValidSudoku();
